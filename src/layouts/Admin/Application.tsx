...
} from "../../app/api/Taskspot/Application";
...
const FileLink = ({ url }: { url?: string }) => {
  ...
  // Cloudinary URLs already start with https:// — no need to prepend base
...
export default function Application() {
  const { data, isLoading, isError } = useGetAllApplicationRequestsQuery();
  ...
  // ✅ Status update — calls PATCH /:id/status
  const handleStatusSave = async (id: number | string, status: string) => {
    await updateRequest({ id, status }).unwrap(); // throws on error — caught in modal
  };
